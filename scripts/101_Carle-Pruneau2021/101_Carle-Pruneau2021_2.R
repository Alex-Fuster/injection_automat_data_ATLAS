###########################################################################
# Injection of Tachycineta bicolor adults time-series into Atlas
...
# Time series only for adults

file_name <- "retrieved_datasets/101/101_natal_dispersal.csv"
...
data_timeseries <- brut |>
  ...
  dplyr::select(c("year.ahy", "idadult"))  |>
  dplyr::group_by(year.ahy) |>
...
time_series <- data_timeseries |>
  ...
  dplyr::rename(years = "year.ahy") |>
  dplyr::mutate(unit = "Number of adults found in nesting boxes")
