###########################################################################
# Injection of Tachycineta bicolor hatchlings time-series into ATLAS
...
# - A timeseries is formated for each of the life-stages (hatchlings, fledlings, female and male adults)
...
# years are labeled from 1 to 7, where 1 = 2005, etc.

brut$Year[which(brut$Year == "1")] <- "2005"
brut$Year[which(brut$Year == "2")] <- "2006"
brut$Year[which(brut$Year == "3")] <- "2007"
brut$Year[which(brut$Year == "4")] <- "2008"
brut$Year[which(brut$Year == "5")] <- "2009"
brut$Year[which(brut$Year == "6")] <- "2010"
brut$Year[which(brut$Year == "7")] <- "2011"
...
data_timeseries <- brut |>
  ...
  dplyr::select(c("Year", "hatchlings")) |>
  dplyr::group_by(Year) |>
  dplyr::summarise(count = sum(hatchlings))
...
time_series <- data_timeseries |>
  ...
  dplyr::mutate(unit = "Number of hatchlings recorded in nest-boxes visited every 2 days throughout the breeding season (April to August)")
...
data_timeseries <- brut |>
  ...
  #dplyr::mutate(`Chrysemys picta` = ifelse(nbr.peinte > 0, nbr.peinte/`nuits-piège`, NA),
  # `Chelydra serpentina` = ifelse(nbr.serpentine > 0, nbr.serpentine/`nuits-piège`, nbr.serpentine)) |>
  dplyr::select(c("Year", "fledglings")) |>
  dplyr::group_by(Year) |>
  dplyr::summarise(count = sum(fledglings))
...
time_series <- data_timeseries |>
  ...
  dplyr::mutate(taxon = stringr::str_to_sentence(taxon)) |>
  dplyr::rename(years = "Year") |>
  dplyr::mutate(unit = "Number of females captured in nest-boxes visited every 2 days throughout the breeding season (April to August)")
